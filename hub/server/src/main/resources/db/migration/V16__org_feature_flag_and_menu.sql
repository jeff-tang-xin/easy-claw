-- ============================================================================
-- V16__org_feature_flag_and_menu.sql
-- 平台级目录模型（菜单/功能开关/工具）——真正的结构变更迁移。
--
-- 历史说明（为什么 V15 与 V16 内容相同）：
--   本内容最初写在 V15__org_feature_flag_and_menu.sql 中，其旧版已于
--   2026-09-21 16:17 应用到本库（checksum 29124556）；随后文件被改写为
--   平台级目录模型但未升版本号，导致 Flyway validate checksum mismatch、
--   hub 无法启动。修复方式（等价 Flyway repair）：
--     1) V15 保留为「已应用占位」，DB 侧 checksum 已对齐为现文件值；
--     2) 全部实际变更由本 V16 承载。
--   对全新库：V15 与 V16 依次执行，V16 的 DROP TABLE IF EXISTS 会清掉
--   V15 刚建的表与种子后重建，最终结构与本库一致（幂等）。
-- ============================================================================
-- 平台级目录模型（菜单/功能开关/工具）：
