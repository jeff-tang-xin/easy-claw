...
public class AgentScopeProperties {
    ...
    /**
     * 按「智能体」粒度覆盖模型配置：key 为 agentId
     * （如 {@code main}、{@code coder}、{@code reviewer}、{@code planner}、
     * {@code researcher}、{@code code-expert}、{@code file-expert}）。
     * <p>
     * 用于让不同智能体走不同的 provider / 模型：例如 coder 用擅长写代码的模型，
     * researcher 用支持联网或上下文更长的模型，而无需改动任何代码。
     * 未在此表登记的智能体沿用全局 {@code agentscope.model} 配置。
     * <p>
     * 对应 yml 形如：
     * <pre>{@code
     * agentscope:
     ...
     * @see #resolveAgentModel(String)
     */
    private Map<String, AgentModelConfig> agents = new LinkedHashMap<>();
     ...
     * 优先级从高到低：
     * <ol>
     *   <li>{@code agents.&lt;agentId&gt;} 的显式字段</li>
     *   <li>{@code agents.&lt;agentId&gt;.provider} 指向的 {@code providers.&lt;provider&gt;}</li>
     *   <li>全局 {@code model}</li>
     * </ol>
     *
     * @param agentId 智能体标识，null/空白或未配置时返回 {@link ModelPreference#DEFAULT}
     * @return 合并后的模型偏好
     */
    public ModelPreference resolveAgentModel(String agentId) {
        if (agentId == null || agentId.isBlank() || agents == null || !agents.containsKey(agentId)) {
            return ModelPreference.DEFAULT;
        }

        AgentModelConfig agentCfg = agents.get(agentId);
        ProviderConfig providerCfg = null;
        if (agentCfg.getProvider() != null) {
            providerCfg = providers.get(agentCfg.getProvider());
        }

        String modelName = agentCfg.getModelName();
        if (modelName == null && providerCfg != null) {
            modelName = providerCfg.getModelName();
        }
        if (modelName == null) {
            modelName = model.getModelName();
        }

        String baseUrl = agentCfg.getBaseUrl();
        if (baseUrl == null && providerCfg != null) {
        ...
        String apiKey = agentCfg.getApiKey();
        if (apiKey == null && providerCfg != null) {
        ...
        return new ModelPreference(modelName, baseUrl, apiKey);
    }
     ...
     * 单个智能体的专属模型配置。
     * <p>
     * 所有字段均可为 null，null 表示「不覆盖，继承上层」：
     * 先回退到 {@link #getProvider()} 指向的 {@link ProviderConfig}，再回退到全局 {@code model}。
     *
     * @see AgentScopeProperties#resolveAgentModel(String)
     */
    public static class AgentModelConfig {
        /** 引用 {@code providers} 表中的某个 key（如 deepseek、openai），null 表示不指定 provider */
        private String provider;

        /** 覆盖模型名，null 表示继承 provider / 全局配置 */
        private String modelName;

        /** 覆盖 base-url，null 表示继承 provider / 全局配置 */
        private String baseUrl;

        /** 覆盖 api-key，null 表示继承 provider / 全局配置 */
        private String apiKey;

        /** 覆盖采样温度，null 表示继承 provider / 全局配置 */
        private Double temperature;

        /** 覆盖单次最大输出 token，null 表示继承 provider / 全局配置 */
        private Integer maxTokens;

        /** 覆盖是否允许并行 tool_call，null 表示继承 provider / 全局配置 */
        private Boolean parallelToolCalls;
    ...
    public Map<String, AgentModelConfig> getAgents() {
        return agents;
    }

    public void setAgents(Map<String, AgentModelConfig> agents) {
        this.agents = agents;
    }
