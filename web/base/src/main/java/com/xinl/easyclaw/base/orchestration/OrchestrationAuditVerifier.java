 ...
 * 编排执行审计校验器。
 * <p>
 * 编排型模式（{@link AgentOrchestrator#requiresExecutionAudit()} 返回 true 的模式）
 * 的控制流由主 LLM 按提示词自行执行，系统侧无法保证其一定按计划走。本类通过解析
 * 主智能体回复末尾的审计标记
 * <pre>&lt;orchestration-audit stages="2" executed="1:planner,2:coder|reviewer" skipped="" /&gt;</pre>
 * 与实际计划比对，把「编排是否真的发生」从不可观测变为可观测。
 * <p>
 * <b>归属说明</b>：审计标记的要求由编排行为规范（{@link OrchestrationBehavior}）
 * 声明，本校验器是同一协议的解析端，故同归编排层；调用方（api 层）按模式声明
 * 决定是否启用校验。
 * <p>
