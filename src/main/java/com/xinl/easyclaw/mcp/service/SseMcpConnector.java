 ...
 * SSE MCP 连接器。
 * <p>
 * 使用 Java MCP SDK 连接 SSE MCP Server，并把远端工具转换为 Embabel Tool。
 */
@Component
public class SseMcpConnector implements McpConnector {

    private static final Logger log = LoggerFactory.getLogger(SseMcpConnector.class);

    private final McpSdkSupport mcpSdkSupport;

    public SseMcpConnector(McpSdkSupport mcpSdkSupport) {
        this.mcpSdkSupport = mcpSdkSupport;
    }

    @Override
    public boolean supports(McpServiceEntity service) {
        return service != null && "SSE".equalsIgnoreCase(service.resolveTransport());
    }

    @Override
    public void connect(McpServiceEntity service) {
        boolean ok = mcpSdkSupport.connect(service);
        log.info("SSE MCP 连接结果: name={}, success={}", service.getName(), ok);
        if (!ok) {
            throw new IllegalStateException("SSE MCP 连接失败: " + service.getName());
        }
    }
